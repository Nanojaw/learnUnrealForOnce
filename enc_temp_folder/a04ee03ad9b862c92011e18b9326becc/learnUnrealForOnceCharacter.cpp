...
void AlearnUnrealForOnceCharacter::Push()
{
	...
	FVector End = Start + (FollowCamera->GetComponentLocation().ForwardVector * 500);
	...
	DrawDebugLine(GetWorld(), Start, End, FColor::Yellow, false, 8, 0, 2.0f);

	auto HitObj = OutHit.GetActor();

	if (HitObj != nullptr) GEngine->AddOnScreenDebugMessage(-1, 5, FColor::Emerald, TEXT("You hit ") + HitObj->GetName());
}
